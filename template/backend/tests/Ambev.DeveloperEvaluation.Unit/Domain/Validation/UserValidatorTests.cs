...
/// password hash, phone, status, and role requirements.
/// </summary>
public class UserValidatorTests
{
    ...
    /// <summary>
    /// Tests that validation fails for invalid password hash formats.
    /// This test verifies that password hashes that don't meet the requirements:
    /// - Must not be empty
    /// - Must be a valid hash format
    /// fail validation with appropriate error messages.
    /// The test uses TestDataGenerator to create invalid password hash formats.
    /// </summary>
    [Fact(DisplayName = "Invalid password hash formats should fail validation")]
    public void Given_InvalidPasswordHash_When_Validated_Then_ShouldHaveError()
    {
        // Arrange
        var user = UserTestData.GenerateValidUser();
